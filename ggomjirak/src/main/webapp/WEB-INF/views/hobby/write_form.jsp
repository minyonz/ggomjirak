...
// 				var epos = ui.item.index();
	...
	function delStepBox(seq) {
		...
		if (length < 5) {
			alert("만들기 순서는 최소 4개가 있어야합니다.");
			return false;
		}
		
		var thisStepBox = $("#stepBox_" + seq);
		if (thisStepBox.find(".stepText").val() != "") {
			if(confirm("입력하신 내용이 있습니다 삭제하시겠습니까?")) {
								...
								<div class="materialBox" id="materialBox_1" style="cursor:pointer; margin: 0 0 7px 50px;">
									...
									<div class="divMaterialItem" id="divMaterialItem_1" style="display:inline-block;">
										...
										<input type="text" style="width:200px; display: inline-block;" class="materialName form-control" 
											name="hobbyMaterials[0].materialName" placeholder="예)비즈"/>
									...
									<div class="none_img stepBox" id="stepBox_1"> <!-- <-- 한줄짜리 스탭을 감싸는 박스 -->
										...
										<div class="divStepItem">
											...
											<div class="divStepUpload" id="divStepUpload_1">
												...
												<div style="position: relative; bottom: 10.5rem;">
...
function delMainImg() {
	...
	$.get(url, function(rData) {
		if (rData == "success") {
			$("#main_img").val("");
			$("#previewImg_main").attr("src", "/resources/images/main_img_btn.jpg");
...
// 	  	$(box).attr("id", "divMaterialItem_" + (i + 1));
...
function validate() {
	...
	if (typeof main_img == "undefined" || main_img.trim() == "" || main_img ==  null) {
		alert("메인이미지 사진을 올려주세요");
		return false;
	} 
	
	var hobby_title = $("#hobby_title").val();
	if (typeof hobby_title == "undefined" || hobby_title.trim() == "" || hobby_title ==  null) {
		alert("hobby_title 입력해주세요");
		console.log(hobby_title);
		return false;
	}
	
	var hobby_intro = $("#hobby_intro").val();
	if (typeof hobby_intro == "undefined" || hobby_intro.trim() == "" || hobby_intro ==  null) {
		alert("error! hobby_intro=" + hobby_intro + " 필수정보입니다");
		return false;
	} 
	
	var videoJ = /(http(s)?:\/\/|www.|)(youtu.+\.*)+[a-z0-9]{2,4}/gi;
	// 			https://www.youtube.com/watch?v=yHjFmePbqmk
// 				https://youtu.be/yHjFmePbqmk
// 				youtu.be/yHjFmePbqmk
//              -> 대충 테스트했을때 이정도가 들어갈 수 있는 url형식들이었음 어차피 유튜브 영상은 뒤에 yhjfme이값만 가져올거기때문에
//                 상관없음..
	...
	if (l_cate_no.trim() == "") {
		alert("error! l_cate_no=" + l_cate_no + " 필수정보입니다");
		return false;
	} 
	
	if (m_cate_no.trim() == "") {
		alert("error! m_cate_no=" + m_cate_no + " 필수정보입니다");
		return false;
	} 
	...
	$(".materialBox").each(function(i, box) {
		var materialName = $(box).find(".materialName").val();
		if (typeof materialName == "undefined" || materialName.trim() == "" || materialName ==  null) {
			alert("값없음 박스있으면 필수정보!, materialName:" + materialName);
			return false;
		} 
	});
	...
	$(".stepBox").each(function(i, box) {
		...
		if (typeof step_img == "undefined" || step_img.trim() == "" || step_img ==  null) {
			alert("값없음 박스있으면 필수정보!, step_img:" + step_img);
			return false;
		} 
		
		// 3-2. 스탭내용
		var step_text = $(".stepText").val();
		if (typeof step_text == "undefined" || step_text.trim() == "" || step_text ==  null) {
			alert("값없음 박스있으면 필수정보!, step_text:" + step_text);
			return false;
		} 
